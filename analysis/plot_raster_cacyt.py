...
def stackplot_with_features(ah,t,y,peaks,rts,fwhms,taus,ipeaks,irts,ifwhms,itaus,hgap,tshift):
    ...
    for i in range(0,len(ipeaks)):
        # plot peaks
        ah.plot(t[ipeaks[i]]+tshift,y[ipeaks[i]]+hgap,marker="o",markersize=0.8,linestyle="",color="red")
        # plot 20-80 risetime
...
folder1 = "data/astron/raw/astrocyte/"
...
# plot scale bar
ahs_raster[0].plot([3,5],[15,15],linestyle="-",marker="",linewidth=0.5,color="black")
ahs_raster[0].text(3,12,"2 sec",font=fontprop,fontsize=10)
ahs_raster[0].plot([5,5],[15,19],linestyle="-",marker="",linewidth=0.5,color="black")
ahs_raster[0].text(6,15,"4 $\mu$M",rotation=90,font=fontprop,fontsize=10)
...
fh1_name = "".join(("dhpg100000nM2s_cacyt_stack_plot_",groups[0],".svg"))

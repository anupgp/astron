...
{  
  ...
  // double calbindin_ca_cyt_flux = newinsilico::neuron_value(index, "ca_cyt_buffer_simple_flux"); /* NOT INCLUDED */
  // double cap_ca_entry_ca_cyt_flux = newinsilico::neuron_value(index, "cap_ca_entry_ca_cyt_flux"); /* NOT INCLUDED */
  // double ryr_xw_ca_cyt_flux = newinsilico::neuron_value(index, "ryr_xw_ca_cyt_flux"); /* NOT INCLUDED */
  ...
  // call writing consitions
  writing_conditions::current(variables, dxdt, t, index);
};
...
astrocyte::astrocyte():
  ...
  // ryrxw(getpid()*std::chrono::system_clock::now().time_since_epoch().count()*564319)  /* NOT INCLUDED */
{};

// Time-stamp: <2017-08-24 12:00:27 anup>
...
// ================================
void generate_values_expdecay(matrix& out_matrix, double tstart, double tstop, double tstep, double curval, double tau, int ncols)
// Generate a matrix of values with time as the first column and values in subsequent columns (ncols)
{
  ...
  std::cout  << std::endl;
  // --------------------
  // Create a vector of exponential decay
  std::vector<double> expdecay;
  for(auto el : times){
    expdecay.push_back(curval * exp(-(el-tstart)/tau));
  }
  for(auto el:expdecay){
    std::cout << el << " ";
  }
  std::cout  << std::endl;
  // -----------------
  row_matrix onerow;
  {
    typename std::vector<double>::iterator it_expdecay = expdecay.begin();
    for(typename std::vector<double>::iterator it_times = times.begin(); it_times < times.end(); it_times++){
      onerow.push_back(*it_times);
      for(int i = 0; i < (ncols-1); i++){
	onerow.push_back(*it_expdecay);
	std::cout << *it_expdecay << " ";
      }
      std::cout << std::endl;
...
int main(int argc, char* argv[])
{
 ...
 // assign parameter names
  std::vector<std::string> pnames{"fname","colname","ncols","tstart","tstop","tstep","pulse-amp","pulse-decaytau","sepchar"};
  ...
  if(!check_parameters(args,pnames)){
    ...
    std::cerr << "USAGE: generate_runtime_mod_file_1param_fixedstep --fname fname --colname1 time --colname2 atp_ext0 --ncols 2 --tstart 10 --tstop 12 --tstep 5e-6 --pulse-amp 10e-6 --pulse-decaytau 2 --sepchar ','" << "\n";   
    return(-1);
  }
  ...
  long double pulse_decaytau = std::stod(args.at("pulse-decaytau"));

  std::cout << "---------------------------------" << std::endl;
  std::cout << "tstart = " << tstart << std::endl;
  std::cout << "tstop = " << tstop << std::endl;
  std::cout << "tstep = " << tstep << std::endl;
  std::cout << "pulse_decaytau = " <<pulse_decaytau << std::endl;
  ...
  if( (outfile.is_open()) == true && (outfile.is_open() == true) ) {
    ...
    generate_values_expdecay(values,tstart,tstop,tstep,pulse_amp,pulse_decaytau,ncol);

// Time-stamp: <2018-04-10 11:01:54 anup>
...
class randomengine
{
  ...
  std::mt19937_64 gen;
  long long seed;
  inline explicit randomengine(long long seed_):
    gen(seed_)
  {};
};
// ================================
template<typename T> struct Distribution{};
template<> struct Distribution<int>{ typedef std::uniform_int_distribution<int> uniform_dist_type;};
template<> struct Distribution<double>{ typedef std::uniform_real_distribution<double> uniform_dist_type;};
// ================================
template<typename T> void random_generator(std::vector<T> &outvar, randomengine randeng, double startval, double stopval)
{
  typename Distribution<T>::uniform_dist_type uni_dist(startval,stopval);
  for(typename std::vector<T>::iterator it = outvar.begin(); it < outvar.end(); it++){
    *it = uni_dist(randeng.gen);
  }
}
// ================================
template<typename T> void random_generator(T &outvar, randomengine randeng, double startval, double stopval)
{
  typename Distribution<T>::uniform_dist_type uni_dist(startval,stopval);
  outvar = uni_dist(randeng.gen);
}
// ================================
void generate_values_poisson(matrix& out_matrix, double tstart, double tstop, double tstep, double pulsedur, double curdur, double curval, int ncols)
// Generate a matrix of values with time as the first column and values in subsequent columns (ncols)
{
  ...
  long long seed = static_cast<long long>(time(NULL)) * getpid();
  randomengine randeng(seed);
  std::vector<double> rvalues(rtimes.size(),0);
  random_generator(rvalues,randeng,0,1);
  // -----------------------
  // for(auto el:rvalues){
  //   std::cout << el << " ";
  // }
  // std::cout  << std::endl;
  // ----------------------
  // Create a vector sampling times
  ...
  row_matrix onerow;
  for(typename std::vector<double>::iterator it = times.begin(); it < times.end(); it++){
    onerow.push_back(*it);
    // Find the index of rtimes >= *it & rtimes < *it + tstep
    typename std::vector<double>::iterator it_low = std::lower_bound(rtimes.begin(),rtimes.end(),*it);
    if(it_low != rtimes.end()){
      unsigned index = it_low - rtimes.begin();
      if((tstep * (1 / pulsedur)) > rvalues[index]) 
	curvalnow = curval;
      else
	curvalnow = 0.0;
    }
    ...
    // std::cout << "time = " << *it << " curval = " << curvalnow << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  // double pulse_on_dur = 1.0;
  // double pulse_period = 1.0;
  // int ncol = 2;
  // generate_values_poisson(values,tstart,tstop,tstep,pulse_amp,pulse_on_dur,pulse_period,ncol);
 // assign parameter names
  ...
  std::cout << "pulse_on_dur = " <<pulse_on_dur << std::endl;
  std::cout << "pulse_period = " <<pulse_period << std::endl;
  std::cout << "pulse_amp = " <<pulse_amp << std::endl;
  ...
  if( (outfile.is_open()) == true && (outfile.is_open() == true) ) {
    ...
    generate_values_poisson(values,tstart,tstop,tstep,pulse_period,pulse_on_dur,pulse_amp,ncol);

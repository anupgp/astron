...
atp_synapse_complete_file="input/synapse/atp_synapse_complete.isf";
glutamate_synapse_complete_file="input/synapse/glutamate_synapse_complete.isf";
...
glusyn_file="input/synapse/glutamate_synapses.isf";
rm -f "$glusyn_file"
touch "$glusyn_file"
count=0;
...
do
    ...
    pre="${pre_glu[$count]}";
    post="${post_glu[$count]}";
    echo "pre: $pre, post: $post, $temp";
    sed  -i "s/pre *: *[0-9]*,/pre : ${pre},/g" "$temp"
    sed  -i "s/post *: *[0-9]*,/post : ${post},/g" "$temp"
    # echo "going to pause 10 secs"
    # pause 10
    cat "$temp" >> "$glusyn_file"; 
    echo "Inserting a new line at the end of the file"
...
atpsyn_file="input/synapse/atp_synapses.isf";
rm -f "$atpsyn_file"
touch "$atpsyn_file"
count=0;
...
echo "*** Generating complete synapses"
synapses_all_file="input/synapse/synapses_complete.isf"

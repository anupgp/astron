// Time-stamp: <2017-10-18 13:49:03 anup>
					      ...
					      stringmap valnames) 
{
  ...
  // Sensor states
  unsigned k0_index = newinsilico::get_neuron_index(index, varnames["syt4_k0"]);
  unsigned k1_index = newinsilico::get_neuron_index(index, varnames["syt4_k1"]);
  unsigned k2_index = newinsilico::get_neuron_index(index, varnames["syt4_k2"]);
  
  unsigned f0_index = newinsilico::get_neuron_index(index, varnames["syt5_f0"]);
  unsigned f1_index = newinsilico::get_neuron_index(index, varnames["syt5_f1"]);
  unsigned f2_index = newinsilico::get_neuron_index(index, varnames["syt5_f2"]);
  unsigned f3_index = newinsilico::get_neuron_index(index, varnames["syt5_f3"]);
  unsigned f4_index = newinsilico::get_neuron_index(index, varnames["syt5_f4"]);
  unsigned f5_index = newinsilico::get_neuron_index(index, varnames["syt5_f5"]);
    
  // Vesicle release & recycling: Kiss-and-run
  unsigned vkdoc_index = newinsilico::get_neuron_index(index, varnames["vkdoc"]);   // Docked vesicles for KR
  unsigned vkrel_index = newinsilico::get_neuron_index(index, varnames["vkrel"]);   // Released vesicles KR
  unsigned vkend_index = newinsilico::get_neuron_index(index, varnames["vkend"]);   // Endocytosis vesicles KR 
  unsigned vkacd_index = newinsilico::get_neuron_index(index, varnames["vkacd"]);   // Reacidified vesicles KR

  // Vesicle release & recycling: Full fusion
  unsigned vfmob_index = newinsilico::get_neuron_index(index, varnames["vfmob"]);   // Mobile vesicles for FF
  unsigned vfrel_index = newinsilico::get_neuron_index(index, varnames["vfrel"]);   // Released vesicles FF
  unsigned vfend_index = newinsilico::get_neuron_index(index, varnames["vfend"]);   // Endocytosis vesicles FF 
  unsigned vfacd_index = newinsilico::get_neuron_index(index, varnames["vfacd"]);   // Reacidified vesicles FF

  // Get all parameter values
  const double volume_cyt = newinsilico::neuron_value(index, "volume_cyt"); // Volume_Cyt (m^3)
  
  const double kkf = newinsilico::neuron_value(index, parnames["syt4_kf"]);
  const double kkb = newinsilico::neuron_value(index, parnames["syt4_kb"]);
  const double kbb = newinsilico::neuron_value(index, parnames["syt4_bb"]);
  const double k0exo = newinsilico::neuron_value(index, parnames["syt4_0exo"]);
  const double kcaexo = newinsilico::neuron_value(index, parnames["syt4_caexo"]);

  const double fkf = newinsilico::neuron_value(index, parnames["syt5_kf"]);
  const double fkb = newinsilico::neuron_value(index, parnames["syt5_kb"]);
  const double fbb = newinsilico::neuron_value(index, parnames["syt5_bb"]);
  const double f0exo = newinsilico::neuron_value(index, parnames["syt5_0exo"]);
  const double fcaexo = newinsilico::neuron_value(index, parnames["syt5_caexo"]);

  const double rmob2doc = newinsilico::neuron_value(index, parnames["rmob2doc"]);
  const double rkend = newinsilico::neuron_value(index, parnames["rkend"]);
  const double rkacd = newinsilico::neuron_value(index, parnames["rkacd"]);
  const double rkacd2mob = newinsilico::neuron_value(index, parnames["rkacd2mob"]);
  
  const double rfend = newinsilico::neuron_value(index, parnames["rfend"]);
  const double rfacd = newinsilico::neuron_value(index, parnames["rfacd"]);
  const double rfacd2mob = newinsilico::neuron_value(index, parnames["rfacd2mob"]);
  ...
  if (initcheck <= 0) {
    variables[k0_index] = variables[vkdoc_index] /(gvars::Av * volume_cyt * 1000);
    variables[f0_index] = variables[vfmob_index] /(gvars::Av * volume_cyt * 1000);
  ...
  newinsilico::neuron_value(index, valnames["krrelrate"], krrelrate);
  newinsilico::neuron_value(index, valnames["ffrelrate"], ffrelrate);
  newinsilico::neuron_value(index, valnames["totalrelrate"], totalrelrate);
};

void transmitter_release_syt4a::poisson_release(state_type &variables, const double t, unsigned index, 
						ar::random_class &rnum1, ar::random_class &rnum2, ar::random_class &rnum3,
						stringmap varnames,
						stringmap parnames,
						stringmap valnames)
{
  const double krrelrate = newinsilico::neuron_value(index, valnames["krrelrate"]); // KR release rate
  const double ffrelrate = newinsilico::neuron_value(index, valnames["ffrelrate"]); // FF release rate
  ...
  newinsilico::neuron_value(index, valnames["krrelflag"], krrelflag);
  newinsilico::neuron_value(index, valnames["krreltime"], krreltime);
    
  newinsilico::neuron_value(index, valnames["ffrelflag"], ffrelflag);
  newinsilico::neuron_value(index, valnames["ffreltime"], ffreltime);
